export default function Footer() {
  return (
    <footer className="bg-black text-gray-400 mt-20 border-t border-white/10">
      <div className="max-w-7xl mx-auto px-8 py-16">
        <div className="grid grid-cols-1 md:grid-cols-4 gap-10">

          {/* Brand */}
          <div className="md:col-span-1">
            <h2 className="text-3xl font-black text-white mb-3">
              Nex<span className="bg-gradient-to-r from-blue-400 to-purple-400 bg-clip-text text-transparent">Mart</span>
            </h2>
            <p className="text-sm text-gray-500 leading-relaxed mb-6">
              Premium fashion for those who dare to stand out. Curated collections, unbeatable prices.
            </p>
            <div className="flex gap-3">
              <a href="https://www.instagram.com/abdu1lahaha" target="_blank" className="bg-white/10 hover:bg-white/20 p-2.5 rounded-xl transition border border-white/10">
                <svg className="w-4 h-4 text-white" fill="currentColor" viewBox="0 0 24 24">
                  <path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zM12 0C8.741 0 8.333.014 7.053.072 2.695.272.273 2.69.073 7.052.014 8.333 0 8.741 0 12c0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98C8.333 23.986 8.741 24 12 24c3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98C15.668.014 15.259 0 12 0zm0 5.838a6.162 6.162 0 100 12.324 6.162 6.162 0 000-12.324zM12 16a4 4 0 110-8 4 4 0 010 8zm6.406-11.845a1.44 1.44 0 100 2.881 1.44 1.44 0 000-2.881z"/>
                </svg>
              </a>
              <a href="https://www.facebook.com/abdu1lahaha" target="_blank" className="bg-white/10 hover:bg-white/20 p-2.5 rounded-xl transition border border-white/10">
                <svg className="w-4 h-4 text-white" fill="currentColor" viewBox="0 0 24 24">
                  <path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z"/>
                </svg>
              </a>
            </div>
          </div>

          {/* Quick Links */}
          <div>
            <h3 className="text-white font-bold mb-5 uppercase tracking-widest text-xs">Shop</h3>
            <ul className="space-y-3 text-sm">
              <li><a href="/" className="hover:text-white transition">All Products</a></li>
              <li><a href="/cart" className="hover:text-white transition">Cart</a></li>
              <li><a href="/orders" className="hover:text-white transition">My Orders</a></li>
              <li><a href="/register" className="hover:text-white transition">Create Account</a></li>
            </ul>
          </div>

          {/* Customer Service */}
          <div>
            <h3 className="text-white font-bold mb-5 uppercase tracking-widest text-xs">Support</h3>
            <ul className="space-y-3 text-sm">
              <li><a href="/faq" className="hover:text-white transition">FAQ</a></li>
              <li><a href="/shipping" className="hover:text-white transition">Shipping Policy</a></li>
              <li><a href="/returns" className="hover:text-white transition">Return Policy</a></li>
              <li><a href="/contact" className="hover:text-white transition">Contact Us</a></li>
            </ul>
          </div>

          {/* Contact */}
          <div>
            <h3 className="text-white font-bold mb-5 uppercase tracking-widest text-xs">Contact</h3>
            <ul className="space-y-3 text-sm">
              <li className="flex items-center gap-2"><span>📧</span> abdullah5053806@gmail.com</li>
              <li className="flex items-center gap-2"><span>📞</span> +92 312 5381350</li>
              <li className="flex items-center gap-2"><span>📍</span> Islamabad, Pakistan</li>
              <li className="flex items-center gap-2"><span>🕐</span> Mon - Sat: 9am - 6pm</li>
            </ul>
          </div>
        </div>

        {/* Bottom Bar */}
        <div className="border-t border-white/10 mt-12 pt-8 flex flex-col md:flex-row justify-between items-center gap-4 text-xs text-gray-600">
          <p>© 2026 NexMart. All rights reserved.</p>
          <div className="flex gap-6">
            <a href="/faq" className="hover:text-gray-400 transition">Privacy Policy</a>
            <a href="/faq" className="hover:text-gray-400 transition">Terms of Service</a>
            <span>Built with Next.js + .NET + PostgreSQL</span>
          </div>
        </div>
      </div>
    </footer>
  );
}